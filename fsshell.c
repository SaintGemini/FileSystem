...
#include "Disk.h" // includes msf.h and freeSpaceAllocation.h
#include "b_io.h"
...
int cmd_cp(int argcnt, char *argvec[])
{
	...
	printf("b_open returned: %d\n", testfs_src_fd);
	testfs_dest_fd = b_open(dest, O_WRONLY | O_CREAT | O_TRUNC);
	//testfs_dest_fd = b_open(dest, O_WRONLY | O_CREAT);
	do
	{
...
int cmd_mv(int argcnt, char *argvec[])
{
	...
	//if directory, use function to move a directory
	if(fs_isDir(argvec[1])){
		return fs_mvDir(argvec[1], argvec[2]);
	}
	//if file, use function to move/rename file depending on what argvec[2] is
	else if(fs_isFile(argvec[1])){
...
int main(int argc, char *argv[])
{
	...
	// init file system
	fs_init();
	printf("-----VCB info-----\nNum of Blocks: %d\nNext available free space: %d\n", VCB->numDataBlocks, VCB->freeBlockIndex);

	// --------------------------------------------------------------------------
	// 		   Testing LBA READ/WRITE HERE

	// TO TEST HEXDUMP
	// ./hexdump ../disk --start 1 --count 1

	//char *bufferTest = malloc(blockSize);

	//char* readTest = malloc(blockSize * 2);
	//printf("-----TEST 1-----\n");
	...
	// int fd = b_open("testFile", O_WRONLY | O_CREAT | O_TRUNC);
	// strcpy(bufferTest, "The Tao that can be told is not the eternal Tao.The name that can be named is not the eternal name.The nameless is the beginning of heaven and earth.The named is the mother of ten thousand things.\n\0");
	// b_write(fd, bufferTest, strlen(bufferTest));
	// -------Test 2--------
	//printf("-----TEST 2-----\n");
	// 1101
	//b_close(fd);
	// fd = b_open("test2", O_WRONLY | O_CREAT);
	// strcpy(bufferTest, "lasidgnlincslidnmpadgfasldgisglsdkjgsligalamcdois\n\0");
	// b_write(fd, bufferTest, strlen(bufferTest));
	//b_close(fd);
	// ------Test 3-------
	// TODO
	// Read from block 1, then write to block 3
	// ------Test 4------
	// printf("-----TEST 3-----\n");
	// // 1100
	// fd = b_open("testFile", O_WRONLY | O_CREAT );
	// strcpy(bufferTest, "This is testing overwrite\n\0");
	// //printf("STRLEN: %d\n", strlen(bufferTest));
	// b_write(fd, bufferTest, strlen(bufferTest));
	// strcpy(bufferTest, "ADDITION TO BUFFER ONE MORE WORD\n\0");
	// b_write(fd, bufferTest, strlen(bufferTest));
	//b_close(fd);
	//printf("NEXT FREE BLOCK TO WRITE TO: %d\n", nextFreeBlock);
	// //printf("%s\n", bufferTest);

	// flipBits(0,1);

	// b_read(2, readTest, 312);
	// printf("%s\n", readTest);
	// printf("%d\n", strlen(readTest));
